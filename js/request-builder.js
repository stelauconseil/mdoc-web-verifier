...
(function () {
    ...
    async function buildRequestByType(requestTypes) {
        ...
        try {
            ...
            if (ra && ra.isEnabled && ra.isEnabled()) {
                ...
                if (addedCount > 0) {
                    log(
                        `🔏 Added Reader Authentication to ${addedCount} document(s)`,
                    );
                } else {
                    log(
            ...
            console.warn(
                "Reader Authentication processing failed:",
                e.message || e,
            );
        }
    ...
    function buildSingleDocRequest(requestType) {
        ...
        if (userRequest) {
            ...
            const nameSpacesObj = userCredentials.toNameSpaces(
                definition,
                mode,
            );
            log(
        ...
        switch (requestType) {
            ...
            case "mdl_full":
                fields = {
                    ...
                    family_name_national_character: false,
                    given_name_national_character: false,
