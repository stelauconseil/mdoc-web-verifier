...
(function () {
  ...
  async function importPkcs8EcPrivateKey(pem, namedCurve) {
    ...
    if (!namedCurve) throw new Error("namedCurve required (P-256 or P-384)");
  ...
  function firstCertDerArray(chainPem) {
    ...
    return blocks.filter((b) => b.type === "CERTIFICATE").map((b) => b.bytes);
  }
  ...
  async function buildReaderAuthenticationBytes(
    ...
    const ra = ["ReaderAuthentication", sessionTranscript, itemsBytesTagged];
  ...
  async function signReaderAuthentication(itemsRequestCbor) {
    ...
    if (!x5.length) throw new Error("No certificate(s) configured for x5chain");
    // Match common wallet examples: single bstr when exactly one cert; array-of-bstr otherwise
    const x5val = x5.length === 1 ? toBstrBytes(x5[0]) : x5.map(toBstrBytes);
