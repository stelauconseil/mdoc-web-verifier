...
(function () {
  ...
  async function extractPublicKeyFromCert(certDer, quiet = false) {
    try {
      ...
      if (!nobleCurveName || !window.nobleCurves[nobleCurveName]) {
        ...
        if (!quiet)
          console.log("Available curves:", Object.keys(window.nobleCurves));
        return null;
      }
  ...
  function derSignatureToRaw(derSig, expectedLength = 64) {
    try {
      ...
      let offset = 2;
      if (derSig[offset] !== 0x02) throw new Error("Invalid DER r component");
      offset++;
      const rLen = derSig[offset++];
      let rBytes = derSig.slice(offset, offset + rLen);
      offset += rLen;

      if (derSig[offset] !== 0x02) throw new Error("Invalid DER s component");
      offset++;
  ...
  function detectCurveFromCertOID(certDer) {
    try {
      const cert =
        certDer instanceof Uint8Array ? certDer : new Uint8Array(certDer);
      const curveOIDs = {
        "P-256": [0x2a, 0x86, 0x48, 0xce, 0x3d, 0x03, 0x01, 0x07],
        "P-384": [0x2b, 0x81, 0x04, 0x00, 0x22],
        "P-521": [0x2b, 0x81, 0x04, 0x00, 0x23],
  ...
  async function verifyCoseSign1(
    ...
    try {
      ...
      function specAlgFromCurve(nobleCurveName) {
        ...
        if (name.includes("ed25519") || name.includes("ed448")) {
          return {
            ...
            hash: "NONE",
            sigLen: 64,
            curveLabel: name.includes("ed448") ? "Ed448" : "Ed25519",
          };
        }
        return out;
      }
      ...
      const aadBytes =
        externalAadBytes instanceof Uint8Array ? externalAadBytes : null;
      ...
      if (!isValid) {
        try {
          ...
          for (const hashName of hashCandidates) {
            try {
              const msgHash = new Uint8Array(
                await crypto.subtle.digest(hashName, sigStructureBytes),
              );
          ...
          if (window.DEBUG_VERBOSE) console.warn("Prehash verify failed:", e);
        }
      }
  ...
  function parseX509Certificate(certBytes) {
    try {
      ...
      console.log(
        ...
        signature[0] === 0x30 ? "(DER SEQUENCE ✓)" : "(NOT DER SEQUENCE!)",
      );
  ...
  async function validateCertificateChain(issuerCertBytes) {
    ...
    try {
      ...
      if (DEBUG_VERBOSE)
        console.log(`  Certificate signature length: ${certSigLength} bytes`);
      ...
      for (const iaca of matchingIACAs) {
        ...
        try {
          ...
          if (DEBUG_VERBOSE)
            console.log(`  🔍 Diagnostic: Verifying TBS structure...`);
          if (DEBUG_VERBOSE)
            console.log(
          ...
          try {
            ...
            try {
              if (
                ...
                typeof curveLib.ProjectivePoint.fromHex === "function"
              ) {
            ...
            if (preHash) {
              if (DEBUG_VERBOSE)
                console.log(`  🧪 Verify: signature vs ${hashFromOID}(TBS)`);
              isValid = curveLib.verify(
            ...
            if (!isValid) {
              try {
                if (DEBUG_VERBOSE)
                  console.log(`  🧪 Verify: signature vs RAW_TBS`);
                isValid = curveLib.verify(
                  parsedCert.signature,
                  parsedCert.tbsCertificate,
                  iacaPublicKey.key,
              ...
              try {
                ...
                if (curveOrder) {
                  ...
                  if (sVal > half) {
                    ...
                    sigNorm.set(sNorm, rBytes.length);
                    if (DEBUG_VERBOSE)
                      console.log(`  🧪 Verify: low-S signature vs RAW_TBS`);
                    isValid = curveLib.verify(
                      sigNorm,
                      parsedCert.tbsCertificate,
                      iacaPublicKey.key,
  ...
  async function verifyIssuerSignedValueDigests(doc, coseSign1) {
    ...
    const normalizeIssuerSignedItemMap = (obj) => {
      try {
        ...
        if (elementIdentifier !== undefined) m.set(2, elementIdentifier);
    ...
    const getDigestFromMap = (digestMap, digestId) => {
      ...
      if (digestMap && typeof digestMap === "object") {
        if (digestMap[digestId] !== undefined) return digestMap[digestId];
    ...
    try {
      ...
      if (!mso) {
        if (!payloadBytes) {
          result.errors.push("MSO payload missing or not a byte array");
          return result;
        }
        try {
          ...
          if (
            ...
            const inner = toUint8(mso.value) || fromJsonBytes(mso.value);
          ...
          result.errors.push(`Unable to decode MSO payload: ${e.message}`);
          return result;
        }
      }
      ...
      if (window.DEBUG_VERBOSE) {
        const keys =
          mso instanceof Map ? Array.from(mso.keys()) : Object.keys(mso || {});
          ...
          const firstDigest =
            firstNs instanceof Map
              ...
              : firstNs
                ...
                : null;
          const firstBytes = toUint8(firstDigest) || fromJsonBytes(firstDigest);
          if (firstBytes) {
      ...
      for (const [nsName, items] of nsEntries) {
        const nsDigests =
          valueDigests instanceof Map
            ? (valueDigests.get(nsName) ?? valueDigests.get(String(nsName)))
            : (valueDigests?.[nsName] ?? valueDigests?.[String(nsName)]);
        ...
        for (const entry of itemsArr) {
          ...
          const expectedBase64 = typeof expected === "string" ? expected : null;
          if (!expectedBytes) {
            if (window.DEBUG_VERBOSE) {
              console.warn(
          ...
          try {
            ...
            if (cb && typeof cb.Tagged === "function") {
              ...
              computedTagged = new Uint8Array(
                await crypto.subtle.digest(hashAlg, taggedBytes),
              );
          ...
          try {
            ...
            if (
              ...
              const normalizedMap = normalizeIssuerSignedItemMap(decodedItem);
              if (normalizedMap) {
                const encoded =
                ...
                const computedCanonical = new Uint8Array(
                  await crypto.subtle.digest(hashAlg, encoded),
                );
                if (bytesEqual(computedCanonical, expectedBytes)) {
                  result.matched++;
                  continue;
                }

                if (typeof cb.Tagged === "function") {
                  const taggedCanonical = new cb.Tagged(24, encoded);
                  const taggedCanonicalBytes = cb.encode(taggedCanonical);
                  const computedTaggedCanonical = new Uint8Array(
                    await crypto.subtle.digest(hashAlg, taggedCanonicalBytes),
                  );
                  if (bytesEqual(computedTaggedCanonical, expectedBytes)) {
                    result.matched++;
                    continue;
                  }
                }
          ...
          if (window.DEBUG_VERBOSE && !loggedFirstMismatch) {
            ...
            try {
              ...
              try {
                expectedBase64Debug =
                  expectedBase64 || bytesToBase64(expectedBytes);
              } catch {}
              try {
                const cb = getCBOR();
                if (
                  ...
                  if (normalizedMap) {
                    ...
                    if (typeof cb.Tagged === "function") {
                      ...
                      computedTaggedCanonicalHex = hex(h2);
                    }
                  }
                }
              } catch {}
              console.log("[valueDigests] First mismatch debug", {
                ...
                issuerSignedItemBytesHex: issuerPreview
                  ? hex(issuerPreview)
                  : null,
                expectedDigestLen: expectedBytes ? expectedBytes.length : null,
                ...
                computedTaggedDigestBase64: computedTaggedBase64,
                computedCanonicalDigestHex: computedCanonicalHex,
                computedTaggedCanonicalDigestHex: computedTaggedCanonicalHex,
                hashAlg,
              });
            } catch {}
          }
      ...
      result.errors.push(`valueDigests verification error: ${err.message}`);
  ...
  function coseKeyToNoblePublicKey(coseKey) {
    ...
    if (!x || !y) throw new Error("Device public key missing x/y coordinates");
  ...
  async function verifyHolderAuthentication(doc) {
    ...
    try {
      ...
      const hasMac =
        rawDeviceAuth &&
        ((rawDeviceAuth instanceof Map && rawDeviceAuth.get("deviceMac")) ||
          (typeof rawDeviceAuth === "object" && rawDeviceAuth.deviceMac));
      if (hasMac) {
        console.log(
      ...
      if (!deviceKey) throw new Error("MSO.deviceKeyInfo.deviceKey missing");
      ...
      if (!deviceAuthCose) {
        ...
        if (window.DEBUG_VERBOSE) {
          ...
          console.log(
            ...
            raw instanceof Map ? raw.get("deviceAuth") : raw?.deviceAuth,
          );
        }
        const rawBytes = unwrapTaggedBytes(raw) || toUint8Shared(raw);
        const rawDecoded = rawBytes ? decodeCborIfBytes(rawBytes) : raw;
        const ds =
          rawDecoded && typeof rawDecoded === "object" ? rawDecoded : null;
        const deviceAuth = ds ? getFieldAnyShared(ds, ["deviceAuth", 0]) : null;
        if (window.DEBUG_VERBOSE) {
          console.log(
            "[mdocAuth] decoded deviceSigned keys:",
            ds instanceof Map ? Array.from(ds.keys()) : Object.keys(ds || {}),
          );
          console.log("[mdocAuth] decoded deviceAuth value:", deviceAuth);
        }
        if (deviceAuth) {
          let cur = deviceAuth;
          ...
          if (Array.isArray(cur)) {
            ...
            const curBytes = unwrapTaggedBytes(cur) || toUint8Shared(cur);
            if (curBytes) {
      ...
      if (!payloadBytesAttached || payloadBytesAttached.length === 0) {
        ...
        const nameSpacesRaw = ds
          ...
          : null;
        const nsRawBytes =
          unwrapTaggedBytes(nameSpacesRaw) || toUint8Shared(nameSpacesRaw);
      ...
      if (deviceAuthCose[3][0] === 0x30) {
        ...
        if (signatureToVerify[0] === 0x30) {
          signatureToVerify = derSignatureToRaw(deviceAuthCose[3], 96);
        }
      ...
      if (cb) {
        ...
        if (typeof cb.encodeCanonical === "function") {
          try {
            stBytesCandidates.push(cb.encodeCanonical(sessionTranscriptItem));
          } catch {}
        }
      }
      ...
      if (
        ...
        throw new Error("SessionTranscript bytes missing for comparison");
      }
      ...
      const matchWrapped = localWrappedCheck
        ? stBytesCandidates.some((b) => bytesEqualStrict(b, localWrappedCheck))
        : false;
