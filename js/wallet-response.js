...
(function () {
    ...
    function convertToJSON(obj) {
        ...
        if (obj instanceof Set)
            return Array.from(obj).map((v) => convertToJSON(v));
    ...
    function buildResponseViewModel(deviceResponse) {
                ...
                if (bytes) {
                    ...
                    const looksLikeIssuerSignedItem = (obj) => {
                        const hasElementIdentifier =
                            getFieldAny(obj, ["elementIdentifier", 0]) !==
                            undefined;
        ...
        const getDigestId = (it) =>
            getFieldAny(it, ["digestID", "digestId", 2]);
            ...
            try {
                ...
                while (offset < u8.length - 2) {
                    if (u8[offset] === 0x5f && u8[offset + 1] === 0x2e) {
                        ...
                        const embedded = (function extractEmbeddedImageFromBDB(
                            bytes,
                        ) {
                            const findAt = (sig) => {
                                for (
                                    let i = 0;
                                    i <= bytes.length - sig.length;
                                    i++
                                ) {
                                    let ok = true;
        ...
        const parseMRZ = (lines) => {
            ...
            try {
                ...
                const normalizeDocumentType = (line1) => {
                    const two = (line1 || "").substring(0, 2);
                    const c1 = two[0] || "";
                    const c2 = two[1] || "";
                    if (c2 === "<") return c1;
                    return (c1 + c2).trim();
        ...
        function jp2ToJpegDataUrl(u8) {
            try {
                ...
                if (
                    ...
                    console.warn(
                        "⚠️ openjpeg returned invalid result:",
                        result,
                    );
                    return null;
                }
        ...
        const valueToEntry = (
            ...
            if (
                options &&
                options.issuerSignedItemBytes instanceof Uint8Array
            ) {
                ...
                try {
                    ...
                    if (
                        ...
                        if (dataUrl) {
                            ...
                            entry.binary.convertedFrom = mimeType;
                            entry.binary.mimeType = "image/jpeg";
                            entry.binary.formatLabel =
                                "JPEG - converted from JPEG2000";
                            entry.binary.dataUri = dataUrl;
                        }
                    } else {
                        if (u8.length > 0) {
            ...
            if (nameSpaces) {
                ...
                for (const [nsName, nsItemsRaw] of nsEntries) {
                    ...
                    if (Array.isArray(nsItems)) {
                        const rawItems = Array.isArray(nsItemsRaw)
                            ? nsItemsRaw
                            : null;
                        for (let idx = 0; idx < nsItems.length; idx++) {
                            ...
                            try {
                                ...
                                const it = unwrapTaggedOrCbor(
                                    normalizeIssuerItem(item),
                                );
                                let elementIdentifier = getFieldAny(it, [
                                    "elementIdentifier",
                                    0,
                                ]);
                                let elementValue = getFieldAny(it, [
                                    "elementValue",
                                    1,
                                ]);
                    ...
                    // Case 2: Map of elementIdentifier -> elementValue or -> IssuerSignedItem
                        ...
                        for (const [k, v] of nsItems.entries()) {
                            try {
                                const rawVal =
                                    nsItemsRaw instanceof Map
                                        ? nsItemsRaw.get(k)
                                        : v;
                                const issuerSignedItemBytes =
                                    extractIssuerSignedItemBytes(rawVal);
                                const it = unwrapTaggedOrCbor(
                                    normalizeIssuerItem(v),
                                );
                                let elementIdentifier = getFieldAny(it, [
                                    "elementIdentifier",
                                    0,
                    ...
                    if (Array.isArray(cose) && cose.length >= 4) {
                        ...
                        if (prot && prot.length > 0) {
                            try {
                                ...
                                protectedData =
                                    dec instanceof Map
                                        ? Object.fromEntries(dec)
                                        : dec;
                            } catch (_) {}
                        }
                        const alg =
                            protectedData[1] ||
                            (unprot instanceof Map
                                ? unprot.get(1)
                                : unprot?.[1]);
                        let algLabel = "Unknown";
                        if (alg === -7) algLabel = "ES256 (ECDSA with SHA-256)";
                        else if (alg === -35)
                            algLabel = "ES384 (ECDSA with SHA-384)";
                        else if (alg === -36)
                            algLabel = "ES512 (ECDSA with SHA-512)";
                        ...
                        try {
                            ...
                            const signedAt = validityInfo
                                ...
                                : null;
                            const digestAlgorithm = mget(mso, [
                                "digestAlgorithm",
                                2,
                            ]);
                            console.log("mso:", mso);
